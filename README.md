...
Потому что Ангулар и Реакт разрабатываются и поддерживаются крупными корпорациями (Google и Facebook соответственно), в которых работают настоящие суровые программисты, а не бородатые хипстеры. К примеру Fiber (будущая имплементация ключевых алгоритмов Реакта) находится в разработке уже 2 с лишним года и пока не торопится в релиз. В то время, как тот же Vue разрабатывается одним СыньЧуньХуанем из Китая, который разрабатывал вторую версию своей либы немногим меньше, чем Гугл разрабатывала свой Ангулар 2.

Для многих Vue привлекателен своей простотой, но осваивать эту библиотеку в первую очередь - туманная затея, слишком много магии, слишком мало джаваскрипта, все те же проблемы, что у Реакта с Ангуларом + сверху пачка своих. Ну а как вспомогательный инструмент Вью - отличная вещь, когда не нужны возможности Реакта или Ангулара.

Касательно остальных "хайповых" библиотек / фреймворков можно сказать одно: вот кто сегодня вспомнит про "сверхперспективные" Ampersand или Aurelia, например? Никто.
...
Общие вопросы и ответы на них, всякие глупости, мемчики и все такое прочее.
...
И да, и нет. Первая спецификация действительно была разработана и утверждена в кратчайшие сроки, и предназначалась для решения примитивнейших задач (оживить картинку, подсветить кнопочки). С тех пор прошло очень много времени, от первой спецификации осталось только название, да парочка досадных багов, которые нельзя пофиксить из-за обратной совместимости (привет, typeof Null). И вообще, современный JS - единственный язык, разработка которого контролируется сурьезными дядечками из этих ваших гуглов и майкрософтов (tc39, ага).
...
![alt text](http://i.imgur.com/YkqnGCr.jpg "Смищнявочка")
