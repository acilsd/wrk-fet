...
Первое и единственное решение, которое нужно принять: на какой фреймворк садиться. На текущий момент (2017) на выбор 2 с половиной стула: [React](https://facebook.github.io/react/), [Angular](https://angularjs.org) и Vue. В тредике мы традиционно советуем Реакт. Как говорится, давайте разберем по частям...

> Я слышал Vue - выбор успешных господ, а еще есть Inferno JS и прочие `Name` JS. Чего то вы темните, ммм?

Для ньюфажика выбора фактически нет: Реакт. Он крайне JS-центричен: это единственная причина, по которой его советуют в тредике и продвинутых сообществах. Копаясь в Реакте вы невольно углубляетесь в понимание JS, а копаясь же в каком-нибудь `либа_нейм` вы просто копаетесь в очередном недофреймворке. Бонусом идет огромное коммьюнити из труеЪ погроммистов, огромное количество вакансий и тот факт, что Реакт сейчас - промышленный стандарт. Плюсом кровавая гэбня из ЛицоКниги скупает мозги по всему миру, и тот же мэйнтейнер Инферно уже сидит в подвале Фейсбука с отобранным паспортом и пишет Fiber (это не шутка).

Теперь Ангулар: первый сдох, учить его смысла нет. Второй получился настолько монструозным, что учить его просто так опять же смысла нет. Только если зовут на работу, где он уже есть и где вам придется писать на нем. К примеру, чтобы написать тудулист на А2 вам придется забацать раза в два больше кода, чем на реакте с редаксом: и это при том, что комбинейшен реакт+редакс сам по себе известен тем, что требует кучу бойлерплейта. Впрочем, поколупаться на досуге никто не мешает.

Вью - неплохая штука для тех, кто:
*  Не знает JS и не хочет его знать, а пилить моднявые интерфейсы ему хочется;
*  Мечтает о том, чтобы пушить изменения прям в стэйт, прям сразу, и чтобы тимлид за это ноги не оторвал;
*  Знает все и просто хочет попилить приложухи, не создавая по 10 экшн криэйторов на каждый инпут;

Ньюфажикам вью скорее навредит, т.к. работа с этим чудесным порождением китайского гения вообще не требует мозга. В треде царит традиционный хейт в стороны вью(дерастов), а в каждом хейте, как известно, есть доля правды. И, опять же: никто не мешает поколупаться на досуге, благо вью понять даже проще, чем предсказать результаты выборов в России (если вы колупаетесь во вью больше недели - у нас для вас плохие новости).
